...
public class Utils {
	...
	public Map<String, String> fontsToValidate() {
		return Map.of(
			"arial", "Arial".toLowerCase(),
			"timesNewRoman", "TimesNewRoman".toLowerCase(),
			"garamond", "Garamond".toLowerCase(),
			"calibri", "Calibri".toLowerCase(),
			"PDType", "PDType".toLowerCase()
			);
	}
	...
	public Map<String, String> getTitlePageFlags() {
		return Map.of(
			...
			"fulfillment", "Submitted in Partial Fulfillment",
			"thesis", "A Thesis in",
			"dissertation", "A Dissertation in",
			"by", "by",
	...
	public Map<String, String> getStudentDetails() throws IOException {
		List<String> titlePage = new ArrayList<>(Arrays.asList(getTitlePage().split(System.getProperty("line.separator"))));
		String date = titlePage.get(titlePage.size() - 1);
		String department = "";
		String thesisDissertation = "";

		for (String studentDetails : titlePage) {
			if (StringUtils.startsWithAny(studentDetails, getTitlePageFlags().get("college"), getTitlePageFlags().get("department"))) {
				department = department.concat(studentDetails);
			}
			if (StringUtils.startsWithAny(studentDetails, getTitlePageFlags().get("thesis"), getTitlePageFlags().get("dissertation"))) {
		...
		String title = getTitlePage().substring(getTitlePage().indexOf(department) + department.length(), getTitlePage().indexOf(getTitlePageFlags().get("thesis")));
		String major = getTitlePage().substring(getTitlePage().indexOf(thesisDissertation) + thesisDissertation.length(), getTitlePage().indexOf(getTitlePageFlags().get("by")));
		String name = getTitlePage().substring(getTitlePage().indexOf(getTitlePageFlags().get("by")) + getTitlePageFlags().get("by").length(), getTitlePage().indexOf(getTitlePageFlags().get("fulfillment")));

		return Map.of("name", name.trim(), "title", title.trim(), "major", major.trim(), "degree", degree.trim(), "date", date.trim());
	}
	...
	public List<String> getDictionary() throws IOException {
		...
		dictionary.addAll(new ArrayList<>(Arrays.asList(getStudentDetails().get("studentName").split(getRegExPattern().get("space+")))));
		dictionary.addAll(new ArrayList<>(Arrays.asList(getStudentDetails().get("title").split(getRegExPattern().get("space+")))));
		return dictionary;
	}
	...
	public String validateSpelling(String page) throws IOException {
		...
		if (!tempPage.isEmpty() && tempPage != null) {
			List<String> tempPagewords = new ArrayList<>(Arrays.asList(tempPage.split(getRegExPattern().get("space+"))));
